    ...
    async def convert_file(
        ...
        try:
            ...
            if options.get('wait_for_selector'):
                logger.info(
                    f"Waiting for selector: {options['wait_for_selector']}")
            ...
            try:
                ...
                logger.info(
                    "Loading indicator hidden, content should be ready")
            except TimeoutError:
                logger.warning(
...
Examples:
  ...
  python html_to_pdf.py input.html output.pdf --header-footer --wait-time 5
    ...
    parser.add_argument(
        '--width', help='Custom page width (e.g., 21cm, 8.5in)')
    parser.add_argument(
        '--height', help='Custom page height (e.g., 29.7cm, 11in)')
    ...
    parser.add_argument('--header-template',
                        help='Custom header HTML template')
    parser.add_argument('--footer-template',
                        help='Custom footer HTML template')

    # Wait options
    parser.add_argument('--wait-time', type=float,
                        help='Wait time in seconds before PDF generation')
    parser.add_argument('--wait-for-selector',
                        help='CSS selector to wait for before PDF generation')
    ...
    parser.add_argument('--custom-css-file',
                        help='Path to custom CSS file for PDF generation')
