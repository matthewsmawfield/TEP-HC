...
def load_citation_metadata():
    ...
    if not citation_file.exists():
        ...
        return {'version': 'v0.6', 'codename': DEFAULT_CODENAME, 'title': REPO_NAME}

    try:
        if yaml:
            with open(citation_file, 'r') as f:
                data = yaml.safe_load(f)
            version_str = data.get('version', 'v0.6')
        else:
            # Parse manually if yaml not available
            with open(citation_file, 'r') as f:
                content = f.read()
            version_match = re.search(r'version:\s*"?([^"\n]+)"?', content)
            version_str = version_match.group(1).strip() if version_match else 'v0.6'
...
def copy_pdf_to_docs(source_pdf: Path, docs_dir: Path):
    ...
    target_name = f"{PAPER_NUMBER}-{REPO_NAME}-{version_str}.pdf"
...
async def generate_pdf(quality: str = 'high', wait_time: float = 5.0, skip_build: bool = False):
    ...
    async with HTMLToPDFConverter() as converter:
        ...
        print(f"   {docs_pdf}")
        print(f"   {base_dir / docs_pdf.name}")
