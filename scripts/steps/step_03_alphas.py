...
class Step03Alphas:
    ...
    def run(self) -> dict:
        ...
        try:
            ...
            print_status(f"  alpha_M = {alpha_M[idx_cmb]:.4e} (should be ~0)", 
                        "SUCCESS" if abs(alpha_M[idx_cmb]) < 0.1 else "WARNING")
            print_status(f"  alpha_T = {alpha_T[idx_cmb]:.4e} (should be ~0)", 
                        "SUCCESS" if abs(alpha_T[idx_cmb]) < 0.1 else "WARNING")
            print_status(f"  c_s^2 = {stability['c_s2'][idx_cmb]:.4f} (must be >= 0)", 
                        "SUCCESS" if stability['c_s2'][idx_cmb] >= 0 else "ERROR")
            
            # GW170817 constraint
            z_gw = 0.01  # Approximately local
            idx_gw = np.argmin(np.abs(z_grid - z_gw))
            print_status(f"\nGW170817 Constraint (z~0):", "TITLE")
            print_status(f"  |c_g - c_gamma|/c ~ |alpha_T|/2 = {abs(alpha_T[idx_gw])/2:.2e}", "INFO")
            print_status(f"  Constraint: < 10^-15", "INFO")
