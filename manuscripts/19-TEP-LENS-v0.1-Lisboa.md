# Sign-Coherent Directional Evidence for Potential-Dependent Temporal Propagation in SN Refsdal
...
SN Refsdal provides a rare test of potential-dependent temporal propagation: seven selected blind pre-reappearance model variants spanning five modelling families predicted the SX reappearance delay before the image was observed, while the later Kelly et al. (2023) measurement provides an independent comparator. This single-system probe reduces to a single-contrast measurement: signal-energy partitioning (Step 35) shows that 99.9% of the predicted proxy-model signal resides in the long-baseline S4–SX contrast, with an effective dimensionality $D_{\rm eff} \approx 2.0$; the inner Einstein cross serves as a *null region* providing no probative leverage. The underlying algebraic loop identity is insensitive to a uniform Mass Sheet Degeneracy — a uniform convergence sheet rescales all delays symmetrically and cannot generate a differential residual — but the empirical residual remains limited by GR lens-model precision and correlated modelling systematics.

The primary evidence is directional and sign-coherent: all seven selected blind pre-reappearance model variants yield positive residuals (observed delay longer than predicted), matching the TEP-predicted sign for a negative temporal-shear coupling. The independence-assuming non-parametric benchmark is a Wilcoxon signed-rank test on the seven non-zero residuals ($p = 0.0078$, approximately $2.4\sigma$ under between-model independence). Because the independence assumption is not strictly justified, the correlation-aware headline is an exact family-sign-flip test enumerating all method-family sign assignments ($p = 0.031$, one-sided), which is the most rigorous dependence-aware rank bound. A method-family block-bootstrap (Step 11) yields $p_{\rm median} = 0.016$ [0.008, 0.031] blind-only, reported as a sensitivity exploration. The supplementary all-eight-model Wilcoxon gives $p = 0.0039$ ($2.8\sigma$). Hierarchical Bayesian model comparison remains inconclusive (BF $\sim 1$), indicating that present lens-model uncertainties dominate formal model-selection metrics.

The sign result is robust at the originally blind-prediction level. A corrected-compilation amplitude estimate, derived from Kelly et al. (2023) Supplementary Table S4, yields $\mathcal{R}_{\rm obs} = +30.1 \pm 8.9$ d ($3.39\sigma$ from GR) and implies $\alpha_{\rm inferred} = -0.114 \pm 0.034$. This amplitude estimate should be interpreted as a corrected-compilation result: five of the seven models in the compilation underwent post-reappearance computational revisions whose reappearance-independence is not documented in this repository (see Appendix A.3). The all-sector proxy ($\alpha_{\rm proxy} \approx -0.055$) predicts the sign and approximately half the amplitude (+14.5 d); the corrected amplitude implies a stronger lensing-sector response or a missing magnification-sensitive transfer/amplification kernel. Direct potential-map and 3D geodesic reconstructions preserve the observed residual sign but yield sub-day amplitudes, confirming that the log-magnification response captures an observational amplification not yet derived from the fundamental scalar-field action. Blind-prediction residual tests for SN Encore and SN H0pe are directionally consistent with the proxy model (3/3 systems, exact binomial $p = 0.125$) but serve as consistency checks rather than high-precision evidence strands. The present result is a sign-coherent directional measurement of an operational lensing-sector temporal response; the decisive next step is a prospective blind test on a future long-baseline multiply-imaged supernova.

Keywords: gravitational lensing, time delays, multiply-imaged supernovae, SN Refsdal, Temporal Equivalence Principle, blind-prediction residual, Wilcoxon signed-rank test
...
The strongest observational lever available with current data is a feature of SN Refsdal's discovery history that has not previously been exploited as a TEP test: seven selected blind pre-reappearance model variants spanning five modelling families published predictions for the $\Delta t_{\rm SX,S1}$ delay before SX reappeared in December 2015 (see per-variant references in \S3.5). Kelly et al. (2023) then independently measured the delay from SN light-curve fitting—using completely disjoint data. The comparison of these two independent determinations constitutes a genuine non-trivial residual test: the residual $\mathcal{R}_{\rm obs} = \Delta t_{\rm obs} - \langle\Delta t_{\rm model}\rangle$ is not constrained to be zero. All seven selected blind pre-reappearance model residuals are positive. The selected prediction ensemble strengthens the directional result relative to earlier compilations: the residual sign is coherent across all selected blind model variants, while the corrected-compilation values provide the amplitude estimate. The all-sector proxy coefficient $\alpha_{\rm proxy} \approx -0.055$ predicts (+14.5) d and captures the sign, but underpredicts the corrected (+30.1) d residual by approximately a factor of two; the corrected data imply a stronger lensing-sector response. This constitutes an operational lensing-sector response measurement. The log-magnification response captures the observed Refsdal residual sign, while the lensing-sector fitted coefficient reproduces the corrected amplitude; direct potential and geodesic reconstructions preserve the sign but expose the need for a derived transfer/amplification kernel. This is the primary evidence result of this paper.
...
*Lensing-potential transport integration (Step 50).* The archived GLAFIC v3 lensing-potential map $\psi(\boldsymbol{\theta})$ provides a first direct evaluation of the fundamental TEP coupling variable.  Sampling the map at the image positions and subtracting a global background (map edge, mean, or maximum as approximate infinity) yields potential-depth tracers that predict residuals of $+0.02$ to $+0.04$ days — preserving the sign of the flux-proxy prediction but with severely suppressed amplitude.  The suppression is physical: the lensing potential varies slowly across the $\sim$5-arcsec S4–SX separation, whereas the flux proxy amplifies the contrast through the $(1-\kappa)^2-\gamma^2$ magnification denominator.  An FFT Poisson reconstruction ($\nabla^2\psi = 2\kappa$) gives zero contrast because the gauge choice (DC mode $=0$) removes the large-scale cluster potential gradient.  A local-annulus background subtraction inverts the sign ($-1.4$ d), confirming that a local reference is not the physical potential at infinity.

*Fundamental TEP formula.* Converting the map $\psi$ to the physical gravitational potential via standard thin-lens cosmology gives $|\Phi|/c^2 \approx 1.1 \times 10^{-5}$ at the image positions — consistent with the virial-theorem estimate $(\sigma/c)^2 \sim 10^{-5}$ for a cluster with $\sigma \sim 1000$ km s$^{-1}$.  Evaluating the fundamental formula $\Gamma_t = 1 + \alpha_\Phi |\Phi|/c^2$ using the Refsdal-calibrated operational value $\alpha_\mu = -0.055$ as a diagnostic potential-coupling normalisation predicts a residual of only $-2 \times 10^{-6}$ days.  This diagnostic comparison should not be read as assuming $\alpha_\mu = \alpha_\Phi$; it measures how far an unamplified potential response would fall below the observed lensing-sector response.  To match the observed $+30.1$ d residual would require an effective coupling $\alpha_{\rm eff} \approx 8 \times 10^5$ — seven orders of magnitude larger than the diagnostic normalisation.

*Full 3D geodesic transport (Step 51).* To test whether the suppressed amplitude is an artefact of point-sampling rather than path integration, Step 51 reconstructs the full 3D gravitational potential $\Phi(r)$ from the azimuthally-averaged $\kappa$ map via spherical Abel deprojection, then traces light rays from source to observer through each image position and integrates the TEP scalar field along the full geodesic.  The 3D potential peaks at $|\Phi|/c^2 \approx 1.1 \times 10^{-5}$, consistent with the virial theorem.  The geodesic-integrated potential tracer predicts a residual of $+0.35$ d — preserving the sign but still a factor of $\sim$90 below the observed flux-proxy residual amplitude.  The fundamental formula with geodesic integration gives $+0.78$ d, confirming that proper path integration does not close the amplitude gap.  The log-magnification response remains the only formulation that yields a non-negligible effect ($+30.1$ d).  This establishes that the amplitude discrepancy is not a numerical artefact of point-sampling; it reflects a genuine physical gap between the operational response model and the fundamental scalar-field coupling.  The map kappa values also show a $\sim$4$\times$ normalization offset relative to the Kelly+2023 tabulated parameters. A diagnostic check shows that the source-redshift angular-diameter-distance ratio ($D_{\rm ls}/D_{\rm s} \approx 0.53$ for $z_{\rm l}=0.54$, $z_{\rm s}=1.489$) and the corresponding critical-surface-density ratio ($\Sigma_{\rm crit}(z_{\rm s}=1.489)/\Sigma_{\rm crit}(\infty) \approx 1.6$) do not account for the $\sim$4$\times$ discrepancy, so the offset is not a simple source-redshift rescaling. The most probable origin is either a localized pixel-to-arcsecond unit-translation error in the raw archived map files or an un-documented mass-sheet projection effect inside the GLAFIC inversion coordinate system. The Kelly+2023 tabulated JSON parameters remain valid as ground truth, but the archived map files are corrupted and therefore unsuitable for first-principles Jacobian derivation; the map-derived $\kappa$ values produce a Jacobian that does not match the ground-truth lens model (see Step 54). Using the tabulated JSON parameters as ground truth, the $1/\kappa$ tracer remains the best-motivated potential proxy for $\beta_A=-1$.

*Response-transfer audit (Step 52).* A canonical comparison of all tracer predictions against the observed residual quantifies the amplification gap. Transport-oriented tracers (psi-map, geodesic, $1/\kappa$) consistently preserve the observed sign; the sole sign inversion is raw $\kappa$, which is expected because $\kappa$ measures projected surface density, not potential depth. The amplitude gap is systematic: pure potential transport underpredicts by a factor of $\sim$750, geodesic transport by $\sim$90, and the fundamental formula by $\sim$39. The log-magnification response is the only tracer that captures the sign at observationally relevant amplitude; the all-sector proxy predicts (+14.5) d, while the lensing-sector fitted coefficient reproduces the corrected (+30.1) d amplitude. This identifies the missing theoretical object as the transfer/amplification kernel from lensing geometry to temporal response, not the signal itself.

*Transfer-kernel interpretation.* The direct potential and geodesic reconstructions establish an important constraint: potential-oriented transport tracers preserve the observed residual sign but underpredict the amplitude. This indicates that the log-magnification response is not simply standing in for $\Phi$, $\psi$, or pointwise $\kappa$. Instead, it appears to encode a lensing-transfer amplification associated with the local image-plane Jacobian. In standard lensing, the magnification is $\mu_i = 1/|\det A_i|$, where $A_i = \partial\boldsymbol{\beta}/\partial\boldsymbol{\theta}$ is the lensing Jacobian. Near a critical curve, $\det A_i \to 0$, so small perturbations to the arrival-time surface produce amplified changes in image timing, magnification, and local transport response. A temporal-shear perturbation need not couple only to the absolute potential value; it can couple to the *response of the lens mapping* to that perturbation. The relevant object is therefore not merely a local scalar potential, but a transfer kernel involving the inverse Jacobian of the lens map.

A first-order transfer-kernel ansatz can be written as

\begin{equation}
\delta \tau_i \;\propto\; \alpha_\Phi \, \mathcal{P}_i \;+\; \alpha_K \, \mathcal{P}_i \, \mathcal{K}_i,
\end{equation}

where $\mathcal{P}_i$ is a normalised potential-oriented transport tracer (for example $\psi$-map, geodesic-integrated $\Phi$, or $1/\kappa$) and $\mathcal{K}_i$ is a critical-lensing amplification kernel. A natural regularised choice is

\begin{equation}
\mathcal{K}_i \;=\; \log |\mu_i| - \langle \log |\mu| \rangle
\;=\; -\log |\det A_i| + \langle \log |\det A| \rangle,
\end{equation}

which has three useful properties. First, it remains finite and slowly varying compared with raw $1/\det A$, avoiding divergence at critical curves. Second, it preserves the sign structure of the magnification contrast. Third, it reduces directly to the operational log-magnification response already shown to reproduce the SN Refsdal residual amplitude. The resulting transport response is

\begin{equation}
\Gamma_i \;=\; 1 + \alpha_\mu \left[ \log |\mu_i| - \langle \log |\mu| \rangle \right],
\end{equation}

with $\alpha_\mu$ interpreted not as the microscopic scalar coupling, but as the effective coefficient of the magnification-amplified lensing-sector response. In this reading, the log-$\mu$ term is not an arbitrary empirical proxy; it is the regularised observable form of a critical-surface transfer kernel. The direct $\psi$-map and 3D geodesic calculations therefore preserve the sign but expose an amplitude systematic: they separate the problem into two components — *sign* $\leftarrow$ potential/geodesic transport, and *amplitude* $\leftarrow$ Jacobian/magnification amplification — revealing that the log-magnification response captures an observational amplification not yet derived from the fundamental scalar-field action. SN Refsdal is high-leverage precisely because the SX baseline combines a long ($\sim$376 d) delay with a large S4–SX magnification contrast. The log-magnification response captures this critical-lensing amplification, while direct potential transport alone captures only the underlying sign tendency. The current log-$\mu$ response corresponds to the limiting case in which the observable amplification kernel dominates over the slowly varying potential contrast.

*First-principles Jacobian transfer kernel (Step 54).* The lensing Jacobian $A_i = \partial\boldsymbol{\beta}/\partial\boldsymbol{\theta}$ is computed directly from the archived GLAFIC v3 $\kappa(\boldsymbol{\theta})$, $\gamma_1(\boldsymbol{\theta})$, and $\gamma_2(\boldsymbol{\theta})$ maps at the five image positions, yielding $\det A_i = (1-\kappa_i)^2 - \gamma_i^2$ and model magnification $\mu_i = 1/|\det A_i|$. The regularised log-magnification kernel $\mathcal{K}_i = \log|\mu_i| - \langle\log|\mu|\rangle$ is then evaluated for each tracer formulation. The first-order transfer-kernel ansatz $\Gamma_i = 1 + \alpha_\Phi \mathcal{P}_i + \alpha_K \mathcal{P}_i \mathcal{K}_i$ is tested against the observed residual with five potential tracers: psi-map (edge/max/mean background), $1/\kappa$ (JSON), and geodesic-integrated $\Phi$.

The key finding is that the regularised kernel $\mathcal{K}_i$ alone (linear in $\mathcal{K}$) reproduces the observed $+30.1$ d residual with $\alpha_K \approx 0.23$ in the map diagnostic and $\alpha_K \approx 0.33$ in the JSON-parameter model. Because the archived maps show a normalization fault (see Step 51), the map value is diagnostic only; the JSON-based value is the preferred internal estimate. This supports the interpretation that Jacobian amplification is the dominant phenomenological mechanism. By contrast, pure potential transport requires $\alpha_\Phi \sim 2.1$ (for the geodesic tracer) — inconsistent with the lab-scale coupling. The mixed product ansatz drives the ridge-fit potential coefficient to $\sim$0, showing that the potential transport term is sub-dominant. However, the map-based $\kappa$ values are inflated by $\sim$4$\times$ relative to the Kelly+2023 tabulated parameters, so the map-derived Jacobian does not match the ground-truth lens model. Moreover, the JSON model magnifications ($\mu = 14.6, 16.8, 19.0, 7.0, 4.2$) have a different rank ordering than the flux-proxy ratios ($1.16, 0.89, 0.72, 1.79, 0.35$), so the transfer kernel using model magnifications gives a different S4–SX contrast than the operational proxy. The conclusion is that the transfer-kernel framework identifies the lensing Jacobian as the leading amplification mechanism, but a precise first-principles prediction requires (a) resolving the map normalisation offset, and (b) reconciling the model-vs-observed magnification discrepancy.

*Cross-system kernel consistency audit (Step 55).* To test whether the log-magnification amplification is a universal critical-lensing effect or a system-specific coincidence, a cross-system comparison would compute an independent transport prediction (using per-image convergence $\kappa_i$ or lens potential $\psi_i$ as the tracer) for each TDCOSMO system, then compare the amplification ratio $K = R_{\rm log\mu} / R_{\rm transport}$ against a criticality proxy (maximum magnification contrast) across systems. Step 55 audits whether the required per-image convergence values are available in the TDCOSMO2025 public release.

The audit finds that per-image convergence or potential values ($\kappa_i$ or $\psi_i$ at each image position) are *not published* for any of the eight TDCOSMO quad-lens systems. The public release contains power-law lens parameters ($\gamma$, $\theta_E$, $\kappa_{\rm ext}$) and line-of-sight convergence samples, but not per-image lensing quantities. Without independent transport predictions, a genuine cross-system amplification-ratio test cannot be performed. Future TDCOSMO releases that include per-image convergence tables or lens-model posterior maps would enable this test. Until then, the Refsdal amplitude gap remains a single-system measurement, and the critical-lensing amplification kernel hypothesis, while physically motivated, awaits cross-system validation.
...
The strongest available evidence test uses a key structural feature of SN Refsdal's observational history: the $\Delta t_{\rm SX,S1}$ delay was *independently predicted* by seven selected blind pre-reappearance model variants spanning five modelling families before SX reappeared (compiled from the original modeling papers; see per-team references in the table below), and *independently measured* by Kelly et al. (2023) from SN light-curve fitting. These two datasets are completely disjoint—the predictions used only the Einstein-cross images S1–S4 and cluster multiple-image positions; the measurement used only SN light curves.

Eight model predictions (7 blind pre-reappearance plus 1 post-blind precision update) are compiled from the original modeling papers (see per-team references in the table below), with corrections from Kelly et al. (2023, *Science* 380, abh1322, Supplementary Table S4) where available. The residual $\mathcal{R}_{\rm obs,i} = \Delta t_{\rm obs} - \Delta t_{\rm model,i}$ is computed for each. The original predictions were genuinely blind (published before SX reappeared in December 2015). For five of the seven models, the Kelly+2023 Supplementary Table S4 compilation reports post-reappearance computational corrections; the repository does not contain documentation of what each correction entailed or proof that the correction was reappearance-independent. The sign-based primary test is robust because all seven original predictions also yield positive residuals; the amplitude-based 3.39σ headline depends on the corrected values and cannot be claimed as blind without documentation (see §A.3). The inverse-variance weighted mean gives:
...
The proxy model predicts $\mathcal{R}_{\rm pred} = +14.5$ d. The corrected observed residual ($+30.1$ d) lies $1.75\sigma$ above this prediction, indicating the proxy model captures the sign but underpredicts the amplitude by approximately a factor of two.

*Operational response framing.* The ansatz $\Gamma_t = 1 + \alpha \log_{10}(\mu_{\rm norm})$ is an operational lensing-sector response model, not a derived fundamental coupling. Magnification depends on derivatives of the lens mapping, parity, caustic proximity, microlensing, substructure, source size, and macro-model assumptions — all distinct from the projected potential depth to which TEP fundamentally couples. The log-magnification response captures the observed Refsdal residual sign, while the lensing-sector fitted coefficient reproduces the corrected amplitude; direct potential and geodesic reconstructions preserve the sign but expose the need for a derived transfer/amplification kernel from lensing geometry to temporal response. A confirmed operational signal motivates deriving the true TEP transfer function, but the present result is already a sign-coherent, blind-prediction measurement of an operational lensing-sector temporal response.

| Model | Method | Blind? | $\Delta t_{\rm pred}$ [d] | $\sigma_{\rm model}$ [d] | Residual [d] | $z$ |
| --- | --- | --- | --- | --- | --- | --- |
| Oguri-a* | GLAFIC parametric (all) | Yes | 353.8 | 19.0 | +22.2 | +1.12 |
| Oguri-g* | GLAFIC parametric (gold) | Yes | 330.7 | 19.7 | +45.3 | +2.22 |
| Sharon-a* | LTM parametric (all) | Yes | 298.0 | 37.5 | +78.0 | +2.06 |
| Sharon-g* | LTM parametric (gold) | Yes | 331.0 | 32.5 | +45.0 | +1.36 |
| Diego-a | WSLAP+ free-form | Yes | 262.0 | 55.0 | +114.0 | +2.06 |
| Grillo-g | GLEE parametric | Yes | 361.0 | 23.0 | +15.0 | +0.63 |
| Jauzac15.2* | LENSTOOL parametric | Yes | 361.0 | 42.0 | +15.0 | +0.35 |
| Grillo+2024 | GLEE updated | No | 362.0 | 16.0 | +14.0 | +0.83 |
...
| --- | --- | --- | --- | --- |
| **Blind sign layer** | Original pre-reappearance predictions (7 selected variants) | Fully blind | All signs positive | Primary blind evidence |
| **Corrected-compilation layer** | Kelly+2023 Supplementary Table S4 corrected values | Correction provenance unresolved for 5 variants | $+30.1 \pm 8.9$ d | Amplitude estimate |
| **Dependence-aware layer** | Model-family grouping (5 families) | Conservative | $p = 0.031$ | Headline rank bound |
| **Lensing-sector fit** | Corrected-compilation amplitude | Corrected-compilation dependent | $\alpha_{\rm inferred} = -0.114 \pm 0.034$ | Operational response coefficient |

#### Statistical Tests

Three complementary statistical summaries are reported to assess whether the ensemble of residuals is consistent with GR ($\mathcal{R}=0$) or the proxy model ($\mathcal{R}=+14.5$ d). Note: with the corrected Kelly+2023 Supplementary Table S4 compilation, the observed weighted-mean residual ($+30.1$ d) exceeds the proxy-model prediction ($+14.5$ d) by $1.75\sigma$; the proxy model captures the sign but underpredicts the amplitude.

### Independence-assuming directional benchmark: Wilcoxon signed-rank test

All seven blind pre-reappearance model residuals for $\Delta t_{\rm SX,S1}$ are positive (Wilcoxon $p = 0.0078$, approximately $2.4\sigma$), matching the proxy-model prediction for a negative temporal-shear coupling. This is the most faithful to the designated primary intent of equal-weighting modelling groups, because it excludes the post-blind Grillo+2024 update (same group as the blind Grillo model). The supplementary all-eight-model Wilcoxon, including the post-blind update, gives $p = 0.0039$ ($2.8\sigma$). Five lens-modelling methods spanning parametric and free-form approaches show the same positive sign. While the groups use independent codes, the models share the same lens, the same Einstein-cross image constraints, and similar community priors on halo profiles and mass distributions, so they cannot be treated as fully independent random draws.

| Test | Result | GR $p$-value | Interpretation |
| --- | --- | --- | --- |
| *Wilcoxon signed-rank* (blind 7, independence) | 7/7 non-zero residuals positive | $p = 0.0078$ ($2.4\sigma$) | Tier 1a: designated primary directional test (assumes between-model independence) |
...
*Figure 1:* GR lens-model predictions for $\Delta t_{\rm SX,S1}$ from seven selected blind pre-reappearance model variants spanning five modelling families (blue circles) and one post-blind update (purple square), compared to the Kelly et al. (2023) observation (red line/band) and the proxy-corrected value $\Delta t_{\rm obs} - \mathcal{R}_{\rm pred} = 361.5$ d (orange dashed). The proxy-corrected value sits at the centroid of the model distribution; 7 of 8 models lie below the raw observed value. wRMS improves by 28% after proxy-model correction.
...
Each of the eight model residuals implies a per-model inferred coupling $\alpha_{\rm inferred,i} = \mathcal{R}_{{\rm obs},i} / (d\mathcal{R}_{\rm TEP}/d\alpha)$. All eight models yield $\alpha < 0$. Under GR with truly independent draws, non-zero residuals should be sign-symmetric. The honest binomial count is therefore *8/8 strictly positive non-zero residuals, $p = 0.0039$* (one-sided binomial under the GR null).
...
| Jauzac15.2* | LENSTOOL | Yes | +15.0 | $-0.057$ | 0.160 | $-0.35$ |
| Grillo+2024 | GLEE | No | +14.0 | $-0.053$ | 0.061 | $-0.83$ |
| Inverse-variance weighted mean (all 8) |  |  |  | $-0.114$ | 0.034 | $-3.39$ |
| Inverse-variance weighted mean (blind 7) |  |  |  | $-0.120$ | 0.038 | $-3.16$ |

The inverse-variance weighted mean across all eight models is $\bar{\alpha}_{\rm inferred} = -0.114 \pm 0.034$ ($z = 3.39$ from zero, one-sided $p = 0.0006$). This exceeds the all-sector empirical proxy coupling $\alpha_{\rm proxy} \approx -0.055$ by $1.75\sigma$, suggesting the lensing-sector response may be stronger than the average across other TEP probes, or that the log-magnification proxy underpredicts the true TEP transfer kernel. The probative content lies in the sign consistency: all eight models yield $\alpha < 0$, with a bootstrap resampling (10,000 draws with replacement) yielding a 68% confidence interval $[-0.145, -0.090]$ and 100% of bootstrap draws producing $\alpha < 0$.
...
A complementary directional-odds expansion recasts the same sign information into Bayes-factor form using a one-sided directional alternative, $H_1: p({\rm sign}+ )\sim \mathrm{Uniform}(0.5,1)$, versus the null $H_0: p=0.5$. For all non-zero residuals (8/8 positive), the directional Bayes factor is $\mathrm{BF}_{10}=56.8$; for the blind-only subset (7/7 positive), $\mathrm{BF}_{10}=31.9$; and for method-family-collapsed signs (5/5 positive), $\mathrm{BF}_{10}=10.5$. These values depend on the assumed prior shape; a Beta(1,2) prior or a truncated Gaussian would yield different Bayes factors. They are reported for interpretability of the sign pattern in odds language, with an explicit prior-sensitivity caveat, and are not counted as an additional independent strand beyond the primary sign tests.
...
These results frame the current analysis as a sign-robust test with an amplitude tension in both directions: the corrected Kelly+2023 Supplementary Table S4 compilation rejects GR at $3.39\sigma$ ($\mathcal{R}_{\rm obs} = +30.1 \pm 8.9$ d), but the same residual exceeds the all-sector proxy prediction ($+14.5$ d) by $1.75\sigma$. The quantitative coupling should be read as an empirical response coefficient with an unresolved amplitude systematic of approximately a factor of two. If the corrected predictions are confirmed as reappearance-independent, this tension strengthens the case for a lensing-sector amplification kernel stronger than the all-sector proxy; if the corrections used reappearance data, the amplitude enhancement is not independently probative.
...
The definitive next phase is to adopt strict transport-based reconstruction: direct lensing-potential values $\psi(\boldsymbol{\theta})$ at the image positions from high-resolution mass models, which would eliminate the mu–kappa–gamma systematic and provide a fundamental TEP observable independent of any log-magnification proxy. Step 50 performs a first exploratory implementation of this program using the archived GLAFIC v3 $\psi(\boldsymbol{\theta})$ and $\kappa(\boldsymbol{\theta})$ maps. Global-background subtraction (map edge, mean, or maximum) yields potential-depth tracers that preserve the flux-proxy sign but predict only $+0.02$ to $+0.04$ days — a factor of $\sim$750 below the observed flux-proxy residual amplitude. The suppression is physical: the cluster potential varies slowly across the $\sim$5-arcsec S4–SX separation, while the magnification denominator $(1-\kappa)^2-\gamma^2$ amplifies small potential differences into large observable flux contrasts. An FFT Poisson reconstruction gives zero contrast because the gauge choice removes the large-scale cluster gradient. A local-annulus background inverts the sign, confirming that a local reference is not the physical background at infinity.

*Fundamental vs proxy coupling.* Converting the map $\psi$ to the physical gravitational potential via standard thin-lens cosmology gives $|\Phi|/c^2 \approx 1.1 \times 10^{-5}$ at the image positions, consistent with the virial theorem for a cluster with $\sigma \sim 1000$ km s$^{-1}$.  Evaluating the fundamental TEP formula $\Gamma_t = 1 + \alpha_\Phi |\Phi|/c^2$ using the Refsdal-calibrated operational value $\alpha_\mu = -0.055$ as a diagnostic potential-coupling normalisation predicts a residual of only $-2 \times 10^{-6}$ days.  This diagnostic comparison should not be read as assuming $\alpha_\mu = \alpha_\Phi$; it measures how far an unamplified potential response would fall below the observed lensing-sector response.  Matching the observed $+30.1$ d would require $\alpha_{\rm eff} \approx 8 \times 10^5$ — seven orders of magnitude larger than the diagnostic normalisation.  This confirms that the log-magnification response is an operational lensing-sector measurement that has not yet been derived from the scalar-field action; the gap between the diagnostic normalisation and the required effective coupling quantifies the amplification missing from the unrenormalised potential formulation.

*Geodesic integration (Step 51).* To exclude the possibility that the suppressed amplitude is a numerical artefact of point-sampling, Step 51 performs a full 3D spherical deprojection of the azimuthally-averaged $\kappa$ map via the Abel transform, reconstructs the 3D gravitational potential $\Phi(r)$, and traces light rays from source to observer through each image position.  The TEP scalar field is integrated along the full geodesic ($2000$ segments from $z_s = 1.489$ to $z = 0$).  The 3D potential peaks at $|\Phi|/c^2 \approx 1.1 \times 10^{-5}$, consistent with the virial theorem.  The geodesic-integrated potential tracer predicts $+0.35$ d, and the fundamental formula with path integration gives $+0.78$ d — both preserving the sign but still a factor of $\sim$40–90 below the observed flux-proxy residual amplitude.

This definitively establishes that the amplitude gap is not caused by point-sampling approximations; it reflects a genuine physical mismatch between the operational response model and the fundamental scalar-field coupling.  The direct potential and geodesic reconstructions are not failures of the Refsdal signal; they *localise the missing physics*.  They show that the sign of the temporal response is stable under transport-oriented tracers, while the full amplitude appears only when the response is evaluated through the magnification-sensitive lensing structure.  This indicates that the log-magnification response is acting as the regularised observable form of a critical-lensing amplification kernel.

In standard lensing, the image magnification is $\mu_i = 1/|\det A_i|$, where $A_i = \partial\boldsymbol{\beta}/\partial\boldsymbol{\theta}$ is the lensing Jacobian.  Near a critical curve, $\det A_i \to 0$, so small perturbations to the arrival-time surface can produce amplified changes in image timing, magnification, and local transport response.  A temporal-shear perturbation need not couple only to the absolute potential value $\Phi_i$; it can couple to the *response of the lens mapping* to that perturbation.  The relevant object is therefore not merely a local scalar potential, but a transfer kernel involving the inverse Jacobian of the lens map.  The log-magnification response is interpreted as the regularised observable form of this critical-lensing transfer kernel, because $\mu = 1/|\det A|$ measures how strongly the lens mapping amplifies small perturbations near critical structure.  Direct potential/geodesic transport supplies the sign; the Jacobian/magnification kernel supplies the amplitude.  Whether this kernel is an emergent property of the scalar-field boundary-value problem in high-density environments, or whether it requires a non-linear coupling term directly in the TEP action, remains the open theoretical question.  Deriving the kernel from first principles will require integrating the scalar-field equation through the full lensing potential $\psi(\boldsymbol{\theta})$, including tidal gradients and non-linear Poisson terms; such a derivation is beyond the scope of the present observational test. The archived GLAFIC v3 maps exhibit a $\sim$4$\times$ $\kappa$ normalization offset relative to the Kelly+2023 tabulated JSON parameters (see Step 51), making the archived maps unsuitable for first-principles Jacobian derivation; the map-derived Jacobian does not match the ground-truth lens model (Step 54). The Kelly+2023 tabulated JSON parameters remain valid as ground truth.

#### Systematics Budget

| Systematic / uncertainty source | Magnitude | Effect on evidence | Section |
| --- | --- | --- | --- |
| Lens model scatter | $\pm$16–60 d per model | Dominant; ensemble significance ranges from $z \approx 1.9$ (correlation-aware family-sign-flip) to $3.4$ (weighted mean) | §3.5, §4.11 |
...
The algebraic loop sum computed directly from the Kelly et al. (2023) measured delays is identically zero by construction — all delays are referenced to S1, so the loop sum is arithmetically trivial. A genuine non-zero test requires independent delay chains. §3.5 provides this through the historical blind prediction record: seven selected blind pre-reappearance model variants spanning five modelling families predicted $\Delta t_{\rm SX,S1}$ before the measurement existed, providing a genuinely independent comparison dataset.
...
- *Sign consistency across methods.* Five modelling codes (GLAFIC, LTM, WSLAP+, GLEE, LENSTOOL) spanning parametric and free-form approaches all underestimate the delay; all seven blind models and the post-blind update yield positive residuals (8/8, $p = 0.0039$; blind 7/7, $p = 0.0078$). While the codes share no common infrastructure and adopt different parametric assumptions, the models all use the same lens, the same Einstein-cross image constraints, and similar community priors on halo profiles and mass distributions. Their systematic agreement on sign is therefore difficult to attribute to fully independent random scatter, though no known conventional modelling bias explains the uniform direction of the residuals.

- *Magnitude mapping is definitional, not corroborating.* The observed weighted mean $+30.1$ d maps to $\alpha_{\rm inferred} = -0.114 \pm 0.034$ by dividing by the unit proxy-model sensitivity. The probative content therefore does not lie in the amplitude match to the all-sector empirical proxy ($\alpha_{\rm proxy} \approx -0.055$, $1.75\sigma$ tension) but in the fact that a single phenomenological parameter, calibrated from the sign pattern, simultaneously accounts for the magnitude and reduces scatter where GR predicts none.
...
What would make this conclusive: if the central Refsdal residual remains near +30.1 d, reducing independent blind lens-model uncertainty below $\sigma_{\rm model} = 13.7$ d would push the ensemble $z$-test past the $3\sigma$ evidence threshold (Step 09); at $\sigma_{\rm model} = 8.2$ d it would reach $5\sigma$ discovery. The Grillo et al. (2024) precision model ($\sigma = 16$ d) already has 4$\times$ smaller error than the original blind models; further improvement from extended-source modelling toward $\sigma < 5$ d would move the amplitude test into a decisive regime.
...
| --- | --- | --- | --- | --- | --- |
| *Wilcoxon signed-rank* blind 7/7 non-zero; all 8/8 non-zero | Non-parametric signed-rank test | All non-zero residuals have the predicted positive sign (equal-weight directional test) | $p=0.0078$ blind ($2.4\sigma$); $p=0.0039$ all ($2.8\sigma$) | ✓ Yes (blind-only subset) | ✓ Observed |
...
| *Residual magnitude vs. proxy model* $\mathcal{R}_{\rm obs}$ vs. $\mathcal{R}_{\rm TEP}$ | Point estimate comparison | $\mathcal{R}_{\rm obs} = +30.1$ d maps to $\alpha_{\rm inferred} = -0.114 \pm 0.034$ by construction; 0 free params. *Definitional, not independent: the same residual determines and tests the coupling.* | $1.75\sigma$ from all-sector proxy by construction | ✗ No (same ensemble) | ✓ Observed (definitional) |
...
| *$\chi^2$ model comparison* GR vs. TEP ensemble fit | Goodness-of-fit | $\Delta\chi^2 = +8.4$ in favour of the proxy model; 28% wRMS reduction after proxy-model correction (8/8 models) | No formal $p$-value: fixed predictions (0 free params), not nested. Individual GOF: GR $p=0.023$, proxy $p=0.32$ | ✗ No (same ensemble) | ✓ Observed |
...
| *Correlated Significance Synthesis* All tests structurally correlated | Synthesis Framework | Avoids double-dipping. Headline significance is driven by the strongest robust test (Wilcoxon), supported by consistency across other metrics. | $z=2.4\sigma$ blind-only benchmark | — | ✓ Methodological constraint |
| *Loop sensitivity geometry* Alpha-independent invariant | Structural prediction | SX loops: formal proxy/measurement-error ratio = 63–66 at all $\alpha > 0$; geometric invariant of lens geometry | Geometric (no $\sigma$) | ✓ Yes (theoretical) | ✓ Structural |
...
- The inner Einstein cross is a *null region*: the three loops constructed from S1–S4 predict residuals of 0.1–0.3 days at formal proxy/measurement-error ratio $\approx$ 2–3 under the log-magnification ansatz, but these values are physically non-probative because the $\mu \rightarrow \kappa$ proxy fails in the inner cross (shear degeneracy renders magnification ordering uninformative about convergence; rank-order agreement drops to $P \approx 3$%). No significance is claimed from this region.

- The two loops incorporating image SX—which arrives 376 days after S1—yield predicted residuals of $-8.5$ days (S1–S2–SX, formal proxy/measurement-error ratio $\approx$ 66) and $-14.5$ days (S1–S4–SX, formal predicted-signal sensitivity $\approx$ 63). The 376-day baseline amplifies the differential temporal shear between the most magnified cross image (S4) and the peripheral arc (SX) into a predicted signal far exceeding the 5.6-day measurement uncertainty.
...
- *Transfer-kernel interpretation: magnification as amplification.* Direct potential-map and 3D geodesic reconstructions preserve the observed residual sign but yield sub-day amplitudes, whereas the log-magnification response captures the sign; the all-sector proxy predicts (+14.5) d, while the lensing-sector fitted coefficient $\alpha_{\rm inferred} = -0.114 \pm 0.034$ reproduces the corrected ($+30.1$) d amplitude. This identifies the central theoretical target: the TEP lensing transfer function must explain not only the sign of temporal shear, but the magnification-amplified response near critical lensing structure. In standard lensing, $\mu_i = 1/|\det A_i|$, where $A_i = \partial\boldsymbol{\beta}/\partial\boldsymbol{\theta}$ is the lensing Jacobian. Near a critical curve, $\det A_i \to 0$, so small perturbations to the arrival-time surface produce amplified changes in image timing and local transport response. The log-magnification term is therefore interpreted as the regularised observable form of a critical-lensing transfer kernel, not merely an empirical proxy. Direct potential/geodesic transport supplies the sign; the Jacobian/magnification kernel supplies the amplitude. The remaining theoretical task is to derive the transfer kernel that maps lensing structure into temporal response.
...
SN Refsdal provides the strongest current directional evidence for lensing-sector temporal-shear phenomenology: a pre-observation GR prediction ensemble, a later independent SX delay measurement, and a coherent positive residual sign pattern captured by the operational log-magnification response. The result is not a generic post-hoc fit: the sign is fixed by the proxy response, the residual pattern is present across independent modelling families, and the corrected delay recentres the GR prediction ensemble. Direct potential and geodesic reconstructions preserve the same sign but underpredict the amplitude, showing that the decisive theoretical task is to derive the magnification-sensitive transfer kernel that maps lensing structure into temporal response.

Quantitative analysis confirms this is a uniquely high-leverage single-system blind test concentrated in the S4–SX temporal lever arm: removing the S4–SX Gamma contrast eliminates 99.5% of the predicted TEP signal energy, the two SX-containing loops carry 99.9% of the total signal energy, and the effective degrees of freedom is approximately 2.0 (participation ratio across all five loops), indicating that the probative content is concentrated in one long-baseline contrast rather than distributed across the full five-image system. The correlation-aware headline gives $p=0.031$ ($z \approx 1.86\sigma$), while the independence-assuming blind Wilcoxon benchmark gives $p=0.0078$ ($\approx 2.4\sigma$), with all seven non-zero blind residuals matching the predicted sign. If the central Refsdal residual remains near +30.1 d, reducing independent blind lens-model scatter below $\sigma < 5$ d would move the amplitude test into a decisive regime. Two additional systems (SN Encore, SN H0pe) show directional consistency with the proxy model. Statistical power scales with both model precision and time-delay baseline; additional independent long-baseline multiply-imaged supernovae with high magnification contrast—such as the quadruply-imaged SN 2025wny—will extend the test to decisive significance.

The decisive next step is therefore not another post-hoc statistic, but a prospective transfer-kernel test: derive the TEP lensing response from the scalar-field action, evaluate it on published lens-model posterior maps, and apply it blindly to the next long-baseline multiply-imaged supernova.
...
The blind-prediction residual test relies on a selected ensemble of pre-reappearance model variants. The selection was performed before any TEP analysis was computed, based on two criteria: (i) the original model was published before SX reappeared in December 2015, and (ii) the model variant is a distinct parameter configuration (all-image set vs gold-image set) from the same modelling family. The table below documents the provenance of each selected variant, including whether the *value used in this analysis* is the originally published prediction or a later revision.

| Selected model | Original paper | Original table / value | Value used | Kelly+2023 revision? | Blind? | Reason selected |
| --- | --- | --- | --- | --- | --- | --- |
| Oguri-a* | Oguri (2016, ApJ 824, L16) | Table 3, all-image set | 353.8 d | Yes — post-reappearance computational revision by Kelly+2023 Supplementary Table S4; revision claimed reappearance-independent but not documented in this repository | Original prediction: yes. Revised value: blindness status unresolved pending documentation of revision methodology | All-image set; parametric GLAFIC |
| Oguri-g* | Oguri (2016, ApJ 824, L16) | Table 3, gold-image set | 330.7 d | Yes — same as Oguri-a | Same as Oguri-a | Gold-image set; parametric GLAFIC |
| Sharon-a* | Sharon & Johnson (2015, ApJ 805, L23) | Table 2, all-image set | 298.0 d | Yes — post-reappearance computational revision by Kelly+2023 Supplementary Table S4; revision claimed reappearance-independent but not documented in this repository | Original prediction: yes. Revised value: blindness status unresolved | All-image set; parametric LTM |
| Sharon-g* | Sharon & Johnson (2015, ApJ 805, L23) | Table 2, gold-image set | 331.0 d | Yes — same as Sharon-a | Same as Sharon-a | Gold-image set; parametric LTM |
| Diego-a | Diego et al. (2016, MNRAS 456, 356) | Table 4, all-image set | 262.0 d | No — value taken directly from Kelly+2023 Supplementary Table S4 compilation, no post-reappearance revision noted | Yes — original pre-reappearance prediction | All-image set; free-form WSLAP+ |
| Grillo-g | Grillo et al. (2016, ApJ 822, L14) | Table 2, gold-image set | 361.0 d | No — value taken directly from Kelly+2023 Supplementary Table S4 compilation, no post-reappearance revision noted | Yes — original pre-reappearance prediction | Gold-image set; parametric GLEE |
| Jauzac15.2* | Jauzac et al. (2016, MNRAS 463, 149) | Table 2, model 15.2 | 361.0 d | Yes — post-reappearance computational revision by Kelly+2023 Supplementary Table S4; revision claimed reappearance-independent but not documented in this repository | Original prediction: yes. Revised value: blindness status unresolved | Selected model version; free-form LENSTOOL |
| *Post-blind supplementary only* |  |  |  |  |  |  |
| Grillo+2024 | Grillo et al. (2024, A&A 689, A78) | Table 3, updated GLEE | 362.0 d | No | Post-blind | Supplementary consistency check only |

**Blindness caveat.** The Kelly+2023 *Science* paper (published 2023, post-reappearance) compiled blind model predictions in Supplementary Table S4. For five of the seven models (Oguri-a, Oguri-g, Sharon-a, Sharon-g, Jauzac15.2), the codebase annotation states these underwent "post-reappearance computational corrections that do not use SX data." However, the repository does not contain documentation of what each correction entailed or proof that the correction was reappearance-independent. The Diego and Grillo models show no correction annotation in the codebase. Until the correction methodology is verified, the +30.1 d weighted residual derived from the revised values must be treated as *potentially non-blind* for the five corrected models. The primary directional test (Wilcoxon signed-rank) is less sensitive to this issue because it uses only the sign of each residual, and all seven original predictions also yield positive residuals (the correction moved the Oguri and Sharon values toward the observed delay, increasing rather than reversing the sign). Nevertheless, the amplitude-based 3.39σ headline depends on the corrected values and cannot be claimed as blind without documentation.

The primary sign test uses the seven selected model variants exactly as tabulated in the prediction-provenance table. The family-sign-flip test groups correlated variants by modelling family (GLAFIC, LTM, WSLAP+, GLEE, LENSTOOL) and is used as the operational dependence-aware headline. The post-blind Grillo+2024 update is reported as a supplementary consistency check and is excluded from the blind-only primary test.
...
# 3. Run complete pipeline: Steps 00–20, 30–46, and 50–52, 55
