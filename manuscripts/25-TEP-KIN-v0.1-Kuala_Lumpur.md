...
These calibrated synthetic tests establish a quantitative detection threshold for the device class. The pipeline is not intrinsically blind to disformal phase perturbations — an injected γ &gtrsim; 1.01 is reliably detected — but the real Zimmermann device shows no such signal at the &sim;1% level. The residual phase shift (σ_{Δθ} ≈ 0.42 rad) is too weak and too correlated by the lock-in filter (n_{eff} = 2) to resolve a unique B(φ) shape from a single line cut. Cross-device replication with stronger signals or sharper boundary gradients is the required next step.
